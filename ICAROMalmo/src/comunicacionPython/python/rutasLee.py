...
class CalculoRutas:
	...
	def generarPuntos(self,(x,y)):
		...
		for i in [-1,1]:
			if x+i >= 0 and x+1 < self.width:
				self.grid["%i"%(x+i)]["%i"%(y)] = self.grid["%i"%(x)]["%i"%(y)]+1;
				self.cola.put((x+i,y))
		for i in [-1,1]:
			if y+i >= 0 and y+1 < self.height:
				if not ((y+j) in self.grid["%i"%(x)].keys()):
					self.grid["%i"%(x)] = {}
		...
		for i in [-1,1]:
			for j in [-1,1]:
				if x+i >=0 and y+j>=0 and x+i<self.width and y+j<self.height and self.inGrid(x+i, y+j):
					if self.grid["%i"%(x+i)]["%i"%(y+j)] >-1 and self.grid["%i"%(x+i)]["i"%(y+j)] < this.grid["%i"%(minx)]["%i"%miny]:
						minx = x+i
						miny = y+j
		return (minx,miny)
	
	def generaRutaEncontrada(self):
		pass
